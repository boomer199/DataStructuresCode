...
while(id_number<820):
	...
	if soup.title is not None:
		...
		for event in soup.find_all('table', {"class" : "table"}):
			for event_name in event.find_all('h5', {"class" : "ng-star-inserted"}):
				text_file.write("Event: %s " % str(event_name.contents[0]))
			for event_time in event.find_all('span', {"class" : "ng-star-inserted"}):
